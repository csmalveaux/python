...
lines["rows"] = rows
lines["cols"] = cols
lines["amplitude"] = np.random.uniform(0.25, 0.75, n)
lines["color"] = np.random.uniform(0.5, 1.0, (n, 4))
lines["color"]= 1.0
lines["selected"] = 0.0
lines["xscale"] = np.random.uniform(1, 25, n)

# Each segment has two extra points for breaking the line strip
positions = lines["position"].reshape(rows * cols, p + 2, 3)
positions[:, 1:-1, 0] = np.tile(np.linspace(-1, +1, p),
                                n).reshape(rows * cols, p)
